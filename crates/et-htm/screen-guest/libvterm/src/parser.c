  ...
  for( ; pos < len; pos++) {
    ...
    case CSI_ARGS:
      ...
      if(c >= '0' && c <= '9') {
        ...
        /* et.rs guest safety: trap rather than overflow signed long. */
        if(vt->parser.v.csi.args[vt->parser.v.csi.argi] > (0x7ffffffeL - (c - '0')) / 10)
          __builtin_trap();
      ...
      if(c == ';') {
        /* Upstream otherwise writes beyond args[CSI_ARGS_MAX]. */
