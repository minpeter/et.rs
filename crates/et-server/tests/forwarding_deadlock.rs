...
fn large_full_duplex_forward_survives_mutual_queue_saturation() {
    ...
    connection
        .shrink_transport_window_for_tests(64 * 1024)
        .unwrap();
...
// This deliberately mirrors an endpoint that stops reading its shared
// transport while the forwarding worker holds a packet. The fixed server must
// continue reading and break the mutual wait even against that peer behavior.
fn run_blocking_on_held_packet_pump(
    ...
    let writer = thread::spawn(move || {
        while let Ok(prepared) = write_rx.recv() {
            prepared
                .send()
                .map_err(|error| format!("transport writer failed: {}", error.into_inner()))?;
        }
        Ok::<(), String>(())
    });
    let deadline = Instant::now() + DEADLINE;
    let mut pending = None;
    let mut received_forward_packets = 0_usize;
    let mut sent_forward_packets = 0_usize;
    let mut received_data_packets = 0_usize;
    let mut received_credit_packets = 0_usize;
    let mut sent_data_packets = 0_usize;
    let mut sent_credit_packets = 0_usize;
    let pump_result: Result<(), String> = (|| loop {
        ...
        let remaining = deadline
            ...
            .ok_or_else(|| {
                format!(
                    "client pump deadline elapsed after receiving {received_forward_packets} \
                     ({received_data_packets} data, {received_credit_packets} credit) and sending \
                     {sent_forward_packets} ({sent_data_packets} data, {sent_credit_packets} credit) \
                     forwarding packets"
                )
            })?;
        ...
        if pending.is_none() && network.contains(PollFlags::IN) {
            while pending.is_none() {
                ...
                if is_forward_packet(packet.header()) {
                    received_forward_packets += 1;
                    if let Ok(data) = PortForwardData::decode(packet.payload()) {
                        received_data_packets += usize::from(data.buffer.is_some());
                        received_credit_packets += usize::from(data.window.is_some());
                    }
                    pending = forwarder
                        .try_receive(packet)
                        .map_err(|error| error.to_string())?;
                } else if packet.header() == et_core::proto::TerminalPacketType::KeepAlive as u8 {
                    ...
                    let prepared = connection
                        .prepare_write_packet(
                            et_core::proto::TerminalPacketType::KeepAlive as u8,
                            &ack,
                        )
        ...
        if forwarding.intersects(PollFlags::IN | PollFlags::HUP) {
            while let Some(packet) = forwarder
                ...
                .map_err(|error| error.to_string())?
            {
                sent_forward_packets += 1;
                if let Ok(data) = PortForwardData::decode(packet.payload()) {
                    sent_data_packets += usize::from(data.buffer.is_some());
                    sent_credit_packets += usize::from(data.window.is_some());
                }
                let prepared = connection
    ...
    let writer_result = writer
        .join()
        .map_err(|_| "transport writer panicked".to_owned())?;
    writer_result?;
    pump_result
}
